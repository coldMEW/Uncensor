...
def test_kaggle_notebook_uses_search_subset_before_full_verification() -> None:
    ...
    assert "FULL_VERIFY_REFUSAL_LIMIT = MIN_EVAL_PROMPTS" in source
    assert "FULL_VERIFY_BENIGN_LIMIT = MIN_BENIGN_CONTROLS" in source
    ...
    assert "MAX_GENERATION_EVALUATIONS = max(512, FULL_VERIFY_REFUSAL_LIMIT * 3 + FULL_VERIFY_BENIGN_LIMIT)" in source
    ...
    assert "final_verify_prompts = eval_prompts[:min(FULL_VERIFY_REFUSAL_LIMIT, len(eval_prompts))]" in source
    assert "final_verify_benign_prompts = benign_control_prompts[:min(FULL_VERIFY_BENIGN_LIMIT, len(benign_control_prompts))]" in source
...
def test_kaggle_notebook_records_benchmark_matrix_and_dataset_scale_status() -> None:
    source = _source()
    assert "from src.benchmark_matrix import build_benchmark_matrix, build_metric_evidence_matrix" in source
    assert "'benchmark_matrix': benchmark_matrix" in source
    assert "'metric_evidence_matrix': metric_evidence_matrix" in source
...
def test_kaggle_notebook_uses_second_stage_candidate_grid_and_run_summary() -> None:
    ...
    assert "candidate_search_enabled = candidate_search_enabled and not partial_signal_plateau_ready" not in source
    assert "Skipping second-stage constrained search after partial-signal plateau" not in source
    assert "candidate_search_should_run = bool(candidate_search_enabled)" in source
